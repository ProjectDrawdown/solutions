...
from tools.vma_xls_extract_hfc_replacement import VMAReader
...
def write_ad(f, wb, outputdir):
    ...
    a = wb['Adoption Data']
    # >>> DMK 08.08.2021 This part is fine. Reads stuff in the header of "Adoption Data"
    ...
    # ad_regions = find_ad_regions(wb=wb)  # find_ad_regions doesn't work well for HFCR
    ad_outputdir = os.path.join(outputdir, 'ad')
    os.makedirs(ad_outputdir, exist_ok=True)
    # extract_source_data works poorly for HFCR. 
    # I'll hardcode the sources to give the behavior I want in the for loop
    # extract_source_data also writes the .csv files in the ad subdirectory
    # I therefore also need to call 
    # outputfile = os.path.join(outputdir, filename)
    # df.to_csv(outputfile, header=True)
    # here and properly construct a df for each case.
    # sources = extract_source_data(wb=wb, sheet_name='Adoption Calcs', regions=ad_regions,
    ...
    df.columns = [*df.columns[:2], "Mean lowGWP"]  # Set name of third column manually
    df['lowGWP-lower'] /= 1000  # Convert 0th column to ktons
    df['lowGWP-upper'] /= 1000  # Convert 1st column to ktons
    df = df.loc[2012:]  # Start at year 2012
    # Grab the values missing for 2051 to 2060
    missing_rows = range(74,84)
    for idx, row in enumerate(missing_rows):
        df['lowGWP-lower'][2051+idx] = a['G'+str(row)].value
        df['lowGWP-upper'][2051+idx] = a['H'+str(row)].value
    # pdb.set_trace()
    source_name = 'Velders_et_al_(2015)'
    cases = {}
    region_list = ['World','OECD90','Eastern Europe','Asia (Sans Japan)','Middle East and Africa','Latin America','China','India','EU','USA']
    for case in df.columns:
        cases[case] = case + '_' + source_name
        filename = get_filename_for_source(cases[case], prefix='ad_')
        cases[case] = filename
        outputfile = os.path.join(ad_outputdir, filename)
        # TODO Create proper dataframe here
        case_df = pd.DataFrame(
            # data=np.zeros((df.shape[0], len(region_list))),
            data=np.nan,
            index=df.index,
            columns=region_list,
            dtype=np.float
        )
        case_df['World'] = df[case]
        case_df.to_csv(outputfile, header=True)
    sources = {'World': cases}
