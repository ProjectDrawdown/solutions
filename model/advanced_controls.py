...
def string_to_emisions_grid_range(text):
    from model import emissionsfactors
    return emissionsfactors.string_to_emissions_grid_range(text)

def string_to_co2eq_conversion(text):
    from model import emissionsfactors
...
class AdvancedControls(ParameterCollection):
    ...
    npv_discount_rate: float = parameterField(
        ...
        excelref="Advanced Controls!B126")

    emissions_use_co2eq: bool = parameterField(default=True,
        title="Use CO2-equivalence for Emissions Calculations", isglobal=True,
        units="True/False",
        excelref="EVs Advanced Controls!B174")

    emissions_grid_source: str = parameterField(default="Meta",
        title="Grid Emissions Model", isglobal=True,
        docstring="""Name of the data source to use for grid emissions data.  Options are 'IPCC Only' for IPCC data, or 'Meta', to
        use the the PD Meta-Analysis""",
        excelref="EVs Advanced Controls!C174",
        verifier=string_to_emissions_grid_source)
    
    emissions_grid_meta_version: str = parameterField(default="current",
        title="PD Grid Emissions Version", isglobal=True,
        docstring="Which version of the PD Grid Emissions assessment to use. 'Current' means use the most recent model.",
        excelref="None")
    ...
    emissions_grid_range: str = parameterField(default="mean",
        ...
        excelref="EVs Advanced Controls!D175",
        verifier=string_to_emisions_grid_range)

    co2eq_conversion_source: str = parameterField(default="AR5 with feedback",
        title="Method of conversion of GHG to CO2-eq", isglobal=True,
        docstring="Accepted values are AR5 with Feedback, AR5 without Feedback, AR4 and SAR",
        excelref='SolarPVUtil "Advanced Controls"!I185',
        verifier=string_to_co2eq_conversion)

    # ##########################################################################################
    # 
    # "Constant" Parameters
    # These parameters aren't used in practice and should probably be removed.
    ...
    ch4_is_co2eq: bool = parameterField(isglobal=True, default=False, excelref='SolarPVUtil "Advanced Controls"!I184')
    ...
    n2o_is_co2eq: bool = parameterField(isglobal=True, default=False, excelref='SolarPVUtil "Advanced Controls"!J184')
    ...
    conv_indirect_co2_is_iunits: bool = parameterField(isglobal=True, default=True, excelref='SolarPVUtil "Advanced Controls"!F184')


    # ##########################################################################################
    # 
    # Solution- and scenario-specific parameters
    ...
    soln_first_cost_efficiency_rate: float = parameterField(default=0.0,
    ...
    conv_first_cost_efficiency_rate: float = parameterField(default=0.0,
    ...
    conv_lifetime_capacity: float = parameterField( 
        ...
        units= 'Functional Units per Implementation Units per Implementation Lifetime',
        docstring= (
            "The average expected number of functional units "
    ...
    conv_avg_annual_use: float = parameterField( 
        ...
        units= 'Functional Units per Implementation Units per Year',
        docstring= (
            "The average annual use of the technology/practice, "
            "in functional units per implementation unit. "
    ...
    soln_lifetime_capacity: float = parameterField( 
        ...
        units= 'Functional Units per Implementation Units per Implementation Lifetime',
        docstring= (
            "The average expected number of functional units generated by the "
            "SOLUTION throughout their lifetime before replacement is required. If no replacement "
            "time is discovered or applicable, then the functional unit capacity over 100 years."
    ...
    conv_var_oper_cost_per_funit: float = parameterField( 
        ...
        units= '',
        docstring= (
            "The annual operating cost per functional unit, derived from the "
    ...
    conv_fixed_oper_cost_per_iunit: float = parameterField( 
        title='CONVENTIONAL Fixed Operating Cost (FOM)',
        units= '(per ha per annum)',
        docstring= (
            "The Operating Cost per functional unit, derived "
            "from the CONVENTIONAL mix of technologies/practices.  In Land "
            "solutions this will usually be expressed as a cost per 'hectare of land'."
    ...
    soln_var_oper_cost_per_funit: float = parameterField( 
        ...
        units= 'US$2014 per Functional Unit',
        docstring= (
            "The annual operating cost per functional unit, derived from the "
    ...
    soln_fixed_oper_cost_per_iunit: float = parameterField( 
        title='SOLUTION Fixed Operating Cost (FOM)',
        units= 'US$2014 per Implementation Unit per Year',
        docstring= ("The annual operating cost per implementation unit derived from "
            ...
            "In the case of Land models, the FOM is expressed as cost per hectare of land."),
        excelref= 'SolarPVUtil "Advanced Controls"!I128; Silvopasture "Advanced Controls"!C92')

    conv_fuel_cost_per_funit: float = parameterField(
        title="CONVENTIONAL Fuel Cost per Functional Unit",
        units="",
        docstring="",
        excelref='SolarPVUtil "Advanced Controls"!K95')

    soln_fuel_cost_per_funit: float = parameterField(
        title="SOLUTION Fuel Cost per Functional Unit",
        units="",
        docstring="",
        excelref='SolarPVUtil "Advanced Controls"!K128')
    ...
    ch4_co2_per_funit: float = parameterField(default=0.0, 
        ...
        units= 'Tons CO2-eq per Functional Unit per Year',
        docstring= ("CO2-equivalent of the CH4 emitted per functional unit, in tons/year."),
    ...
    n2o_co2_per_funit: float = parameterField(default=0.0, 
        ...
        units= 'Tons CO2-eq per Functional Unit per Year',
        docstring= ("CO2-equivalent of the N2O emitted per functional unit, in tons/year."),
    ...
    # ##########################################################################################    
    # Solution TAM Parameters

    pds_source_post_2014: str = parameterField(
        title="Data Source for estimation of Scenario Total Addressable Market (PDS TAM) post 2014",
        docstring="""Name of one of the previously-defined TAM data sources, or one of 'ALL SOURCES', 'Baseline Cases',
        'Conservative Cases', 'Ambitious Cases', or '100%Res2050 Case'""",
        excelref='SolarPVUtil "Advanced Controls"!B55')

    ref_source_post_2014: str = parameterField(
        title="Data Source for extimation of Reference Total Addressable Market (Ref TAM) post 2014",
        docstring="""Name of one of the previously-defined TAM data sources, or one of 'ALL SOURCES', 'Baseline Cases',
        'Conservative Cases', 'Ambitious Cases', or '100%Res2050 Case'""",
        excelref='SolarPVUtil "Advanced Controls"!B54')

    source_until_2014: str = parameterField(
        title="Data Source for Total Addressable Market prior to 2014",
        docstring="""Data describing the historical market size prior to 2014.  Name of one of the previously-defined TAM data sources, 
        or one of 'ALL SOURCES', 'Baseline Cases', 'Conservative Cases', 'Ambitious Cases', or '100%Res2050 Case'""",
        excelref='SolarPVUtil "Advanced Controls"!B53')

    ref_tam_custom_source: str = parameterField(
        title="Source of user-supplied Reference TAM",
        docstring="Filename or URI for a user-supplied CSV defining the Ref TAM for all years",
        excelref="None")

    pds_tam_custom_source: str = parameterField(
        title="Source of user-supplied Scenario (PDS) TAM",
        docstring="Filename or URI for a user-supplied CSV defining the PDS TAM for all years",
        excelref="None")

    # ##########################################################################################    
    # Scenario Adoption Parameters

    # ===> I got to here before running out of energy to clean up and/or supply metadata fields
    # ===> Someday to continue.
    ...
    soln_ref_adoption_basis: str = parameterField()
    soln_ref_adoption_custom_name: str = parameterField()
    ...
    soln_pds_adoption_basis: str = parameterField()
    soln_pds_adoption_custom_name: str = parameterField()
    ...
    soln_pds_adoption_scenarios_included: list[int] = parameterField()
    ...
    soln_pds_adoption_custom_high_sd_mult: float = parameterField(default=1.0)
    soln_pds_adoption_custom_low_sd_mult: float = parameterField(default=1.0)
    ...
    soln_pds_adoption_prognostication_source: str = parameterField()
    soln_pds_adoption_prognostication_trend: str = parameterField()
    soln_pds_adoption_prognostication_growth: str = parameterField()
    ...
    ref_adoption_use_pds_years: typing.List[int] = parameterField()
    pds_adoption_use_ref_years: typing.List[int] = parameterField()
    ...
    pds_base_adoption: typing.List[tuple] = parameterField()
    ref_base_adoption: typing.Dict = parameterField()
    pds_adoption_final_percentage: typing.List[tuple] = parameterField()
    ...
    #   factor used in the Bass Diffusion S-Curve model.
    #   SolarPVUtil "ScenarioRecord" rows 170 - 179.
    pds_adoption_s_curve_innovation: typing.List[tuple] = parameterField()
    pds_adoption_s_curve_imitation: typing.List[tuple] = parameterField()
...
@dataclasses.dataclass(repr=False,eq=False,frozen=True)
class LandAC(AdvancedControls):
    # Additional Fields that apply only to Land Scenarios
    # LAND only
   
    tco2eq_reduced_per_land_unit: float = parameterField( 
    ...
    tco2eq_rplu_rate: str = parameterField()
    tco2_rplu_rate: str = parameterField()
    tn2o_co2_rplu_rate: str = parameterField()
    tch4_co2_rplu_rate: str = parameterField()

    tco2_reduced_per_land_unit: float = parameterField( 
    ...
    tn2o_co2_reduced_per_land_unit: float = parameterField( 
    ...
    tch4_co2_reduced_per_land_unit: float = parameterField( 
    ...
    emissions_use_agg_co2eq: bool = parameterField()
    ...
    seq_rate_global: float = parameterField( 
    ...
    seq_rate_per_regime: typing.Dict = parameterField()

    degradation_rate: float = parameterField( 
    ...
    disturbance_rate: float = parameterField( 
    ...
    global_multi_for_regrowth: float = parameterField()
    ...
    soln_expected_lifetime: float = parameterField()
    conv_expected_lifetime: float = parameterField()

    # yield_from_conv_practice: conventional yield in DM tons fodder/ha/year.
    #   Can alternatively be set to 'mean', 'high' or 'low' of its corresponding VMA object
    #   "Advanced Controls"!G77 (Land models)
    yield_from_conv_practice: float = parameterField( 
    ...
    yield_gain_from_conv_to_soln: float = parameterField( 
    ...
    use_custom_tla: bool = parameterField(default=False)

    # a fixed value to set in all elements of a custom TLA, instead of loading from CSV.
    custom_tla_fixed_value: float = parameterField()

    # harvest_frequency: new growth is harvested/cleared every ... (years)
    #   Afforestation "Advanced Controls"!B187
    harvest_frequency: float = parameterField()

    carbon_not_emitted_after_harvesting: float = parameterField( 
    ...
    avoided_deforest_with_intensification: float = parameterField( 
    ...
    delay_protection_1yr: bool = parameterField()
    delay_regrowth_1yr: bool = parameterField()
    ...
    include_unprotected_land_in_regrowth_calcs: bool = parameterField()
    ...
    land_annual_emissons_lifetime: bool = parameterField()

    # MISSING DESCRIPTION
    tC_storage_in_protected_land_type: float = parameterField( 
