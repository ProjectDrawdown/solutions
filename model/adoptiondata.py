...
class AdoptionData:
  """Implementation for the Adoption Data module."""
  ...
  def adoption(self):
    ...
    """
    filename = os.path.join(self.solution_dir, 'solarpvutil_adoptiondata.csv')
    result = pd.read_csv(filename, header=0, index_col=0, skipinitialspace=True, comment='#')
    result.index = result.index.astype(int)
    return result

  def min_max_sd(self, adoption):
  ...
  def low_medium_high(self, adoption, min_max_sd, sourcename, low_sd, high_sd):
    """Return the selected data sources as Medium, and N stddev away as Low and High.
       ...
       sourcename: the name of the source to select from adoption.
    ...
    result.loc[:, 'Medium'] = adoption.loc[:, sourcename]
    result.loc[:, 'Low'] = result.loc[:, 'Medium'] - (min_max_sd.loc[:, 'S.D'] * low_sd)
    result.loc[:, 'High'] = result.loc[:, 'Medium'] + (min_max_sd.loc[:, 'S.D'] * high_sd)
    return result

  def linear_growth(self, adoption):
    """Linear growth model.
  ...
  def poly_degree2_growth(self, adoption):
  ...
  def poly_degree3_growth(self, adoption):
  ...
  def exponential_growth(self, adoption):
