...
VMAs = {
  ...
  'CONVENTIONAL First Cost per Implementation Unit': vma.VMA(
      filename=DATADIR.joinpath(*('energy', 'vma_CONVENTIONAL_First_Cost_per_Implementation_Unit_2.csv')),
      use_weight=True),
  ...
  'CONVENTIONAL Fixed Operating Cost (FOM)': vma.VMA(
      filename=DATADIR.joinpath(*('energy', 'vma_CONVENTIONAL_Fixed_Operating_Cost_FOM_3.csv')),
      use_weight=True),
  ...
  'Coal Plant Efficiency': vma.VMA(
      filename=DATADIR.joinpath(*('energy', 'Coal_Plant_Efficiency_for_wind_solns.csv')),
      use_weight=False),
  'Natural Gas Plant Efficiency': vma.VMA(
      filename=DATADIR.joinpath(*('energy', 'vma_Natural_Gas_Plant_Efficiency_2.csv')),
      use_weight=False),
  'Oil Plant Efficiency': vma.VMA(
      filename=DATADIR.joinpath(*('energy', 'vma_Oil_Plant_Efficiency_3.csv')),
      use_weight=False),
  'Learning Rate (%)': vma.VMA(
      filename=THISDIR.joinpath("vma_data", "Learning_Rate.csv"),
      use_weight=False),
  'Share of Onshore wind on total Wind Generation': vma.VMA(
      filename=THISDIR.joinpath("vma_data", "Share_of_Onshore_wind_on_total_Wind_Generation.csv"),
      use_weight=False),
}
