...
from clusters import paper

test_cluster = paper.Cluster()
test_dfs = test_cluster.run_cluster()

expected_df = pd.read_csv('expected_paper.csv', header=None)
...
def test_paper():
    ...
    pd.testing.assert_frame_equal(test_ref1_tam_low_edu, exp_ref1_tam_low_edu, check_exact=False, rtol=1e-3)
    ...
    print("Test complete: paper cluster")
