"""Biochar solution model.
   Excel filename: Drawdown_RRS_Model_v1.1b_Biochar_Jan2020.xlsm
"""
...
VMAs = {
    ...
    'N2O-CO2eq Tons Reduced': vma.VMA(
        filename=None, use_weight=False),
    'Biochar retail price (US$/tons)': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Biochar_retail_price_US_tons.csv"),
        use_weight=False),
    'Biochar wholesale price (US$/tons)': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Biochar_wholesale_price_US_tons.csv"),
        use_weight=False),
    'Net Biochar Production per Biochar Facility': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Net_Biochar_Production_per_Biochar_Facility.csv"),
        use_weight=False),
    'SOLUTION Net Profit Margin': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "SOLUTION_Net_Profit_Margin.csv"),
        use_weight=False),
    'Biochar potential based on biomass availability by type (crop residues)': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Biochar_potential_based_on_biomass_availability_by_type_crop_residues.csv"),
        use_weight=False),
    'Biochar potential based on biomass availability by type (biomass crops)': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Biochar_potential_based_on_biomass_availability_by_type_biomass_crops.csv"),
        use_weight=False),
    'Biochar potential based on biomass availability by type (other residues)': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Biochar_potential_based_on_biomass_availability_by_type_other_residues.csv"),
        use_weight=False),
    'Carbon content in biomass (%)': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Carbon_content_in_biomass.csv"),
        use_weight=False),
    'Estimated Fraction of Input Biochar that is output to be Sold': vma.VMA(
        filename=THISDIR.joinpath("vma_data", "Estimated_Fraction_of_Input_Biochar_that_is_output_to_be_Sold.csv"),
        use_weight=False),
}
...
class Scenario(scenario.Scenario):
    ...
    def __init__(self, scenario=None):
        ...
        ad_data_sources = {
            'Baseline Cases': {
                'International Biochar Initiative': THISDIR.joinpath('ad', 'ad_International_Biochar_Initiative.csv'),
            },
        }
