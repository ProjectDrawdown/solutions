...
def test_animation_file(tmpdir):
    ...
    efr.process_ghgs(file2020=filename, file2017=filename, outdir=str(tmpdir),
            writer=writer, ext='.gif')
    ...
    for opt in range(1, 4):
        ...
        outfile = tmpdir.join(f"excel_fair_results_test_Concentration_OPT{pds}.csv")
        assert outfile.size() > 1024
        contents = outfile.open().read()
        assert "Emissions (GtC)" in contents
        assert "Baseline (ppm)" in contents
